...
public class ConsumerDelayExample {
    ...
    private static final String delayConsumerGroup = "test_delay_topic_consumer_group";

    public static void main(String[] args) throws ClientException, IOException, InterruptedException {
        final ClientServiceProvider provider = ClientServiceProvider.loadService();
        // 接入点地址，需要设置成Proxy的地址和端口列表，一般是xxx:8081;xxx:8081。
        String endpoints = ServerConfig.ENDPOINTS;
        String topicName = ServerConfig.DELAY_TOPIC_NAME;
        ...
        PushConsumer pushConsumer = provider.newPushConsumerBuilder()
                ...
                .setConsumerGroup(delayConsumerGroup) // 设置消费者分组，需在rocketmq中预先配置
                .setSubscriptionExpressions(subscriptionExpressionMap)// 设置预绑定的订阅关系。
                .setConsumptionThreadCount(1) // 消费者线程数。默认20
                .setMaxCacheMessageCount(1) // 本地缓存消息数量 默认1024，
                ...
                .setMessageListener(messageView -> {
                    ...
                    System.out.printf("[%s] consume delay message successfully, messageId=%s, content=%s \n", Thread.currentThread().getName(), messageView.getMessageId(), mb);
